...
async function main () {
  ...
  await DAOTokenInteract.connect(addr1).approve(Dao.address, amtToApprove);
  await DAOTokenInteract.connect(addr2).approve(Dao.address, amtToApprove);
  await DAOTokenInteract.connect(addr3).approve(Dao.address, amtToApprove);
  await DAOTokenInteract.connect(addr4).approve(Dao.address, amtToApprove);
  const stableApprove5 = await DAOTokenInteract.connect(addr5).approve(Dao.address, amtToApprove);
 ...
 await USDCInteract.connect(addr1).approve(Dao.address, amtToApprove);
 await USDCInteract.connect(addr2).approve(Dao.address, amtToApprove);
 await USDCInteract.connect(addr3).approve(Dao.address, amtToApprove);
 await USDCInteract.connect(addr4).approve(Dao.address, amtToApprove);
 const usdcapprove5 = await USDCInteract.connect(addr5).approve(Dao.address, amtToApprove);
 ...
 await USdcmintingInteract.connect(addr1).mintToken();
 await USdcmintingInteract.connect(addr2).mintToken();
 await USdcmintingInteract.connect(addr3).mintToken();
 await USdcmintingInteract.connect(addr4).mintToken();
 await USdcmintingInteract.connect(addr6).mintToken();
 const addr5mint = await USdcmintingInteract.connect(addr5).mintToken();
 console.log("address 5 minted: ", addr5mint);

  /****************************DAO Interact********************************** */
 const DaoInteract = await ethers.getContractAt("DAO", Dao.address);
 await DaoInteract.connect(addr1).joinDAO("Isaac")
 await DaoInteract.connect(addr2).joinDAO("Sayrarh")
 await DaoInteract.connect(addr3).joinDAO("Abiodun")
 await DaoInteract.connect(addr4).joinDAO("Isiak")
 const ayodejiJOined = await DaoInteract.connect(addr5).joinDAO("Ayodeji")
 console.log("Ayodeji Joined", ayodejiJOined)

  /**************************approve user******************************* */
 await DaoInteract.connect(contractOwner).approveApplicant(addr1.address)
 await DaoInteract.connect(contractOwner).approveApplicant(addr2.address)
 await DaoInteract.connect(contractOwner).approveApplicant(addr3.address)
 await DaoInteract.connect(contractOwner).approveApplicant(addr4.address)
 const ayodejiApprove = await DaoInteract.connect(contractOwner).approveApplicant(addr5.address)
 console.log("Ayodeji approve", ayodejiApprove)



  /****************************create a proposal*********************************** */
 await DaoInteract.connect(addr1).createProposal(
    "Help the needy",
    "Help the needy in my local community",
    5000,
    6
    )
 const addr2Create = await DaoInteract.connect(addr2).createProposal(
    "Funds for Orphans",
 ...
 console.log("proposal 2 approved: ", approveProposal2)
  ...
  /***********************Vote For Proposal 1********************** */
 await DaoInteract.connect(addr1).voteProposal(1)
 await DaoInteract.connect(addr2).voteProposal(1)
 await DaoInteract.connect(addr3).voteProposal(1)
 const approval1_4 = await DaoInteract.connect(addr4).voteProposal(1)
//await DaoInteract.connect(addr5).voteProposal(1)
 ...
 //console.log("approval 1vote 5: ", approval1_5) rrevert with quorum reached
    ...
    /***********************Vote For Proposal 2********************* */
 await   DaoInteract.connect(addr1).voteProposal(2)
 await   DaoInteract.connect(addr2).voteProposal(2)
 await   DaoInteract.connect(addr3).voteProposal(2)
const approval2_4 = await   DaoInteract.connect(addr4).voteProposal(2)
// await   DaoInteract.connect(addr5).voteProposal(2)
 ...
 //console.log("approval 2vote 5: ", approval2_5) revert with qorum reached
    ...
    /******************************************************* */
    const proposal1Details = await DaoInteract.connect(addr2).callStatic.viewProposal(1)
    const proposal2Details = await DaoInteract.connect(addr2).callStatic.viewProposal(2)
    ...
    /**********************Get crowdfunf owner*************************************** */
    const crowdfundOwmner = await crwodfundInteract1.connect(addr1).callStatic.crowdFundOwner()
    const crowdfundOwmner2 = await crwodfundInteract1.connect(addr2).callStatic.crowdFundOwner()
   ...
   //  beneficiary 
   //  cat
    const createCrowd = await crwodfundInteract1.connect(addr1).createCrowdFund( proposal1Details[1], Number(proposal1Details[4]).toString(), proposal1Details[3], 1)
    const createCrowd2 = await crwodfundInteract1.connect(addr2).createCrowdFund(proposal2Details[1], Number(proposal2Details[4]).toString(), proposal2Details[3], 1)
    console.log("crowdfund created: ", createCrowd)
    console.log("crowdfund created2: ", createCrowd2)


    /*************************Get crowdfund Description*********************8 */
     const desc1 = await crwodfundInteract1.connect(addr4).Description()
     const desc2 = await crwodfundInteract2.connect(addr4).Description()

     console.log("descption 1: ", desc1);
     console.log("description 2: ", desc2);

     /***********************Get balance of Donor********************* */
     const balOfDOnor = await USDCInteract.connect(addr6).balanceOf(addr6.address);
    console.log("balance: ", Number(balOfDOnor).toString());

    /**************************Get usdc address********************* */
    const returnusdcaddr = await crwodfundInteract2.connect(addr4).callStatic.USDC()
    console.log("address of susdc: ", returnusdcaddr)

    /**************************Approve crowdfund address************* */
    const approveAmt = ethers.utils.parseEther("500")
    await USDCInteract.connect(addr6).approve(crwodfundInteract1.address, approveAmt);
    const approvalSuccess = await USDCInteract.connect(addr6).callStatic.approve(crwodfundInteract2.address, approveAmt);
    console.log("approval successful: ", approvalSuccess)
    

    /*************************Donate********************* */
    //const dontateAmt =  ethers.utils.parseEther("5")
    const donate = await crwodfundInteract1.connect(addr6).donateFund("1");

    console.log("Donated sucessfullyy: ", donate)

    /*****************all donors address********************** */
  
   const donors = await crwodfundInteract1.connect(addr4).allDonorsAddress();
   console.log("DONORS: ", donors)

    /*******************Amount Donated******************* */
   const amtDonated =  await crwodfundInteract1.connect(addr4).amountDonated(addr6.address);
